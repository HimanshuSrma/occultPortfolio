...
import { LoShuNumber, vastuLoShuGrid, excessNumberVastuDoshDirection, readingGuide, multiYearBlocks, yearlyMonths, monthNames,reportYears } from '../../../models/numerology.model';
...
@Component({
  selector: 'app-result',
  standalone: true,
  imports: [CommonModule, NgxPrintModule, ResultDisplayComponent, FormsModule, PrintComponent],
...
export class ResultComponent implements OnChanges {
  ...
  async ngOnInit(): Promise<void> {
    if(reportYears.includes(this.requestDataType)){
      this.numYears = Number(this.requestDataType);
      this.requestDataType = 'full-report';
      this.generateReportForSpecificTime();
    }else{
      this.numYears = 0;
    }
  ...
  async processLoShuGridForRepeatedNumbers() {
    let arr = [];
    this.vastuGreyAreaDirection = this.result.missingNumbers.map((num: any) => vastuLoShuGrid[num as keyof typeof vastuLoShuGrid]);
    for (const digitStr in this.result.loShuGrid) {
      ...
      if (this.result.loShuGrid[digitKey] > 2) {
        this.vastuGreyAreaDirection.push(excessNumberVastuDoshDirection[digitKey as keyof typeof excessNumberVastuDoshDirection]);
  ...
  generateReportForSpecificTime(): void {
      this.yearlyMonths = [];
  ...
  toggle(group: any, index: number): void {
    group = group.toString()
    this.openIndices[group] = this.openIndices[group] === index ? null : index;
  }
  
  isOpen(group: any, index: number): boolean {
    group = group.toString()
  ...
  // Tracks open sections per month index
  openSections: { [monthIndex: number]: { [sectionIndex: number]: boolean } } = {};

  reportToggle(monthIndex: number, sectionIndex: number): void {
    if (!this.openSections[monthIndex]) {
      this.openSections[monthIndex] = {};
    }

    // Toggle the clicked section
    this.openSections[monthIndex][sectionIndex] = !this.openSections[monthIndex][sectionIndex];
  }

  reportIsOpen(monthIndex: number, sectionIndex: number): boolean {
    return !!this.openSections[monthIndex]?.[sectionIndex];
  }
  ...
  handleRequestedData(data: any) {
    ...
    this.requestDataInResult.emit('null');
    console.log('Received from child in result:', data);
  }
